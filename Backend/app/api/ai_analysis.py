...
async def analyze_with_openai(report_data: AIReportData) -> OpenAIAnalysisResponse:
    ...
    # 프롬프트 생성
    prompt = f"""전력 사용 데이터를 간단히 요약해주세요.

데이터:
- 이상치: {report_data.total_anomaly_count}건
{_format_anomalies(report_data.anomalies[:3])}

- 대기전력 낭비: 월 {report_data.total_standby_waste_kwh} kWh ({report_data.total_standby_waste_cost:,}원)
{_format_standby_devices(report_data.standby_power_devices[:3])}

마크다운 없이 자연스럽게 2-3문장으로 요약하고, 간단한 조언 2-3개, 절약 가능 금액을 알려주세요. 총 200자 이내로 짧게 작성하세요."""
